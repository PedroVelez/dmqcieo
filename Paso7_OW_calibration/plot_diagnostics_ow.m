...
if(isempty(find(isnan(PRES)==0, 1))==0) % if no data exists, terminate here, no plots will be produced
    ...
    if(isempty(find(isnan(mapped_sal)==0, 1))==0) % if mapping exists, terminate here, no plots will be produced
        
        lo_float_calib_data = load( fullfile( po_system_configuration.FLOAT_CALIB_DIRECTORY, pn_float_dir,strcat( po_system_configuration.FLOAT_CALIB_PREFIX, pn_float_name, po_system_configuration.FLOAT_CALIB_POSTFIX ) ) );
        ...
        lo_float_calseries = load( fullfile( po_system_configuration.FLOAT_CALIB_DIRECTORY, pn_float_dir,strcat( po_system_configuration.FLOAT_CALSERIES_PREFIX , pn_float_name, po_system_configuration.FLOAT_MAPPED_POSTFIX ) ) );
        ...
        load(fullfile(po_system_configuration.FLOAT_CALIB_DIRECTORY, pn_float_dir,strcat( po_system_configuration.FLOAT_CALIB_PREFIX, pn_float_name, po_system_configuration.FLOAT_CALIB_POSTFIX ) ),'-regexp','^sta')
        ...
        if( max(LONG)+30>360 | min(LONG)-30<0 ) % if there are float data within 30 deg lon of the prime meridian
            if( isempty( find(LAT<-50&LONG>250&LONG<285, 1) )==0 )
        ...
        for i=1:n
            h=plot(LONG(i),LAT(i),'o','markersize',2);set(h,'color',c(i,:));
        end
        for i=1:10:n
            j=text(LONG(i),LAT(i),int2str(PROFILE_NO(i)));
            set(j,'color','k','fontsize',08,'hor','cen');
        end
        ...
        for j=1:length(ii)
            ...
            c=char(num2str(xticks(ii(j))),enough'); %keyboard
        ...
        print('-dpdf',strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_01_Map','.pdf'));
        ...
        ok = [];
        for k=1:length(jj)
            if ~isempty(find(isfinite(SAL(:,jj)), 1))
                ok = [ok jj(k)];
            end
        end
        jj = ok;
        ...
        set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
        print('-dpdf',strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_02_TS','.pdf'));
        
        %% Calibration curve (figure 3) --------------------------
        if(isempty(find(isnan(cal_SAL)==0, 1))==0) % if no cal exists, terminate here, no plot will be produced
            Soffset=cal_SAL-SAL;
            ...
            subplot(2,1,1)
            plot(PROFILE_NO, pcond_factor, 'b-'); hold on; grid on
            ...
            subplot(2,1,2)
            plot(PROFILE_NO, avg_Soffset, 'b-'); hold on; grid on
            ...
            drawnow
            set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
            print('-dpdf',strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_03_PcDs','.pdf'));
            save(strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_03_PcDs'),'avg_Soffset')
            ...
            colormap(jet(n));
            c=colormap;
            qq=plot( PTMP(:,jj), cal_SAL(:,jj) );grid on
            for i=1:length(jj)
                set(qq(i),'color',c(jj(i),:)) ;
            end
            hold on;
            ...
            set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
            print('-dpdf',strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_04_TSCorr','.pdf'));
            ...
            print('-dpdf',strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_05_SecSalOnTheta.pdf'));
            ...
            save(strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_03_SALint'),'Sint')
            figure
            set(gcf,'defaultaxeslinewidth',2)
            set(gcf,'defaultlinelinewidth',2)
            set(gcf,'defaultaxesfontsize',16)
            for k=1:p
                ...
                if(isempty(Sint)==0)
                    ...
                    hold on
                    plot(PROFILE_NO,Smap(j,:),'r');grid on
            ...
            drawnow
            set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.75,8,9.5]);
            print('-dpdf',strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_06_SOnTheta.pdf'));
            ...
            drawnow
            set(gcf,'papertype','usletter','paperunits','inches','paperorientation','portrait','paperposition',[.25,.5,8,10]);
            print('-dpdf',strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_07_SecSalOnThetaCor.pdf'));
            ...
            % plot s var on t
            subplot(221)
            plot(var_s_Thetalevels,Thetalevels,'b-');hold on
            x = get(gca,'xlim');
            for i=1:10
                plot(x,[tlevels(i) tlevels(i)] ,'g-');
            end
            ...
            print('-dpdf',strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY,filesep,pn_float_name,filesep,po_system_configuration.ANALYSIS_CODE,pn_float_name,'_08_AVariance.pdf'));
            %ps2pdf('psfile', strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY, pn_float_dir, pn_float_name, '.ps'), 'pdffile', strcat(po_system_configuration.FLOAT_PLOTS_DIRECTORY, pn_float_dir, pn_float_name, '.pdf'), 'gspapersize', 'a4', 'deletepsfile', 1)
