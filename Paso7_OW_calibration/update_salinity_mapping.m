...
fprintf('    > UPDATE_SALINITY_MAPPING\n')
...
map_use_pv = str2double( po_system_configuration.MAP_USE_PV );
map_use_saf = str2double( po_system_configuration.MAP_USE_SAF );
...
if exist( ls_float_mapped_filename, 'file' )
    ...
    fprintf('    > loading precalculated mapped data %s \n',ls_float_mapped_filename)
    ...
    fprintf('    > Max number of historical casts used in OM %d\n',ln_max_casts)
    fprintf('    > PV constraint %d SAF separation criteria %d\n',map_use_pv,map_use_saf)
    fprintf('    > Spatial decorrelation scales, in degrees lon (%4.2f,%4.2f), lat(%4.2f,%4.2f) \n',longitude_large,longitude_small,latitude_large,latitude_small),
    fprintf('    > Cross-isobath scales, dimensionless, (%4.2f,%4.2f) \n',phi_large,phi_small)
    fprintf('    > Temporal decorrelation scale, %d years\n',map_age)
    fprintf('    > Exclude the top %d dbar of the water column\n',map_p_exclude)
    fprintf('    > Only use historical data that are within +/- %d dbar from float data\n',map_p_delta)
...
for i = 1 : length( missing_profile_index )
    fprintf('    > UPDATE_SALINITY_MAPPING: Working on profile %d/%d \n',i,length( missing_profile_index))
    ...
    if( isnan(LONG)==0 & isnan(LAT)==0 & isempty(find(isnan(PRES)==0))~=1 )
        ...
        if( la_grid_lat~=999 ) % if no historical data is assigned to the float profile
            ...
            %AQUI
            %save(sprintf('RetrRegionOw_%03d',i),'la_bhist_sal','la_bhist_ptmp','la_bhist_pres','la_bhist_lat','la_bhist_long','la_bhist_dates')
    ...
    fprintf('    > Pres      ')
    fprintf('%6.2f ',PRES(1:10:end))
    fprintf('\n')
    fprintf('    > Sal mapped')
    fprintf('%6.2f ',la_mapped_sal(1:10:end,ln_profile_index))
    fprintf('\n')
