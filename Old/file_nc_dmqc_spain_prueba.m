...
for i=1:length(rep)
    ...
    %nc=ncread([root_out file_out],'write');
   ...
   psalad=netcdf.getVar(ncid,netcdf.inqVarID(ncid,'PSAL_ADJUSTED'))';
keyboard
    ...
    format_v=ncread([root_out file_out],'FORMAT_VERSION');
...
netcdf.putVar(ncid,netcdf.inqVarID(ncid,'PSAL_ADJUSTED'),salad_sp);

ncwrite([root_out file_out],'PSAL_ADJUSTED', salad_sp,[1 1]);
    ...
    %    scientific_calib_equation(index_VSC,1,1:l_eq)=equation; %?me cargo la variable ind_psal? Est? vac?a.
    %    old_coeff=ncread([root_out file_out],'SCIENTIFIC_CALIB_COEFFICIENT');
    %    old_coeff=old_coeff(index_VSC,1,:);
    ...
    %     l_eq=length(equation);
    %     scientific_calib_equation=ncread([root_out file_out],'SCIENTIFIC_CALIB_EQUATION');
    %     scientific_calib_equation(index_VSC,1,ind_temp,1:l_eq)=equation;
    %
