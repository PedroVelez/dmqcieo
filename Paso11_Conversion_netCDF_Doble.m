...
for iboya=1:length(floats)
    ...
    float=floats(iboya);
    root_in=['/Volumes/GDOYE$/Proyectos/Argo/DelayedMode/Data/float_sourceQC1/'  num2str(float) '/profiles/'];
    ...
    for i=1:length(rep);
        ...
        %         if C_FILE.Profs(i).n_prof==1;% Cambio de n_prof=1 a n_prof=2 (conversion a 3.1 - IF)
        %            C_FILE.Profs(i).vertical_sampling_scheme=C_FILE.Profs(i).vertical_sampling_scheme';
        %         end
        ...
        ncwrite([root_out file_out],'PROFILE_TEMP_QC',profile_temp_qc_sp);
