function flt = wrt_flt_gdac(flt)
    ...
    for i = 1:size(flt,2)
        %Check if eddited
        if flt(i).editted
            flname = sprintf('%c%s_%3.3d.nc',flt(i).type,deblank(flt(i).wmo_num'),flt(i).cycle_number);
...
function wrt_flt_nc_prof(flt,ncfile)
    ...
    netcdf.putVar(ncid,varid,flt.temp_qc);
    
    varid=netcdf.inqVarID(ncid,'PSAL_QC');
    netcdf.putVar(ncid,varid,flt.psal_qc);
