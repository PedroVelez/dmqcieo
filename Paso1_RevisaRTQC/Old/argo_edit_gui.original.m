...
% Last Modified by GUIDE v2.5 03-Mar-2011 10:49:22
...
set(handles.text2,'string',[sprintf('Float: %s',flt(1).wmo_num),char(10),...
    sprintf('Profiles: %d',length(flt)),char(10),...
...
for i = 1:length(flt)
    if i == ii-1 | (ii == 1 & i ==2)
        plot(getfield(flt,{i},prop2),getfield(flt,{i},prop1),'m-')
    else
        plot(getfield(flt,{i},prop2),getfield(flt,{i},prop1),'c-')
    end
    hold on
end

if ii == 1
    plot(getfield(flt,{2},prop2),getfield(flt,{2},prop1),'m-')
else
...
i123 = getfield(flt(ii),[prop,'_qc']) == '1' | getfield(flt(ii),[prop,'_qc']) == '2' | getfield(flt(ii),[prop,'_qc']) == '3'; %All good profiles

h3  = plot(getfield(flt(nbr),prop),flt(nbr).pres,'m-','linewidth',1.5);hold on  % neighboring profile
h1a = plot(getfield(flt(ii),prop,{i4}),flt(ii).pres(i4),'ro','markersize',5,'markerfacecolor','r','markeredgecolor','r');
h2a = plot(getfield(flt(ii),prop,{i2}),flt(ii).pres(i2),'o','markersize',5,'markerfacecolor',[1 .5  0],'markeredgecolor',[1 .5  0]);%orange
h3a = plot(getfield(flt(ii),prop,{i3}),flt(ii).pres(i3),'o','markersize',5,'markerfacecolor',[1 .4 .1],'markeredgecolor',[1 .4 .1]);%orange-red
% h2a = plot(getfield(flt(ii),prop,{i2}),flt(ii).pres(i2),'o','markersize',5,'markerfacecolor',[1 .5 0]); %orange
% h3a = plot(getfield(flt(ii),prop,{i3}),flt(ii).pres(i3),'o','markersize',5,'markerfacecolor',[1 .4 .1]);%orange-red

PresAProf=flt(ii).pres(i123);
PropAProf=getfield(flt(ii),prop,{i123});
...
switch popup_sel_index
    case 1
        if isfield(flt(1),'minT') && isfield(flt(1),'maxT') & length(getfield(flt(ii),prop,{i1}))>1
    ...
    case 2
        if isfield(flt(1),'minS') && isfield(flt(1),'maxS') & length(getfield(flt(ii),prop,{i1}))>1
...
if length(oprop_data)>1
    add_plot(oprop_data,flt(ii).pres(i1),'g-');
end
...
set(handles.text3,'string',num2str(newprofile),'fontsize',9)
...
if current_bin > length(flt(i2).pres)
    set(handles.text4,'string',num2str(length(flt(i2).pres)),'fontsize',9)
end
